...
class CustomPlayer {
  ...
  _buildDOM() {
    ...
    this.backBtn.innerHTML = `<svg viewBox="0 0 24 24"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>`;
    ...
    // Picture-in-Picture
    this.pipBtn = this._btn('pip', `<svg viewBox="0 0 24 24"><path d="M19 7h-8v6h8V7zm2-4H3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16.01H3V4.98h18v14.03z"/></svg>`);
    this.pipBtn.addEventListener('click', () => this._togglePip());
    this.buttons.appendChild(this.pipBtn);
  ...
  _buildSettingsMenu() {
    ...
    mainPanel.className = 'cp-settings-main';

    // Quality item
    const qualityRow = document.createElement('div');
    qualityRow.className = 'cp-settings-row';
    const currentQ = this._qualityLabel || 'Auto';
    qualityRow.innerHTML = `
      <svg class="cp-row-icon" viewBox="0 0 24 24"><path d="M3 17v-2h2v2H3zm0-4v-2h2v2H3zm0-4V7h2v2H3zm14 8v-2h2v2h-2zm0-4v-2h2v2h-2zm0-4V7h2v2h-2zm-4 8v-2h2v2h-2zm0-4v-2h2v2h-2zm0-4V7h2v2h-2zM7 17v-2h2v2H7zm0-4v-2h2v2H7zm0-4V7h2v2H7z"/></svg>
      <span class="cp-row-label">${chrome.i18n.getMessage('quality') || 'Quality'}</span>
      <span class="cp-row-value">${currentQ}</span>
      <svg class="cp-row-chevron" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>
    `;
    ...
    speedRow.className = 'cp-settings-row';
    const speedLabel = this._playbackRate === 1 ? (chrome.i18n.getMessage('normal') || 'Normal') : this._playbackRate + 'x';
    speedRow.innerHTML = `
      <svg class="cp-row-icon" viewBox="0 0 24 24"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"/></svg>
      <span class="cp-row-label">${chrome.i18n.getMessage('playbackSpeed') || 'Playback speed'}</span>
      <span class="cp-row-value">${speedLabel}</span>
      <svg class="cp-row-chevron" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>
    `;
    speedRow.addEventListener('click', () => this._showPanel('speed'));
    mainPanel.appendChild(speedRow);

    this.settingsMenu.appendChild(mainPanel);

    // Quality panel
    const qualityPanel = document.createElement('div');
    qualityPanel.className = 'cp-settings-sub cp-hidden';
    ...
    qualities.forEach((q) => {
      ...
      `;
      if (!isActive) { item.style.opacity = '0.4'; item.style.cursor = 'default'; }
      qualityPanel.appendChild(item);
    });

    // Speed panel
    const speedPanel = document.createElement('div');
    speedPanel.className = 'cp-settings-sub cp-hidden';
    const speeds = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2];
    speeds.forEach((s) => {
      const isActive = s === this._playbackRate;
      const label = s === 1 ? (chrome.i18n.getMessage('normal') || 'Normal') : s + 'x';
      const item = document.createElement('div');
      item.className = 'cp-settings-item' + (isActive ? ' active' : '');
      item.innerHTML = `
        <svg class="cp-check" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
        <span class="cp-item-label">${label}</span>
      `;
    ...
    this.settingsMenu.appendChild(qualityPanel);
    this.settingsMenu.appendChild(speedPanel);
    this._qualityPanel = qualityPanel;
    this._speedPanel = speedPanel;
    this._mainPanel = mainPanel;
  }

  _showPanel(name) {
    this._mainPanel.classList.add('cp-hidden');
    this._qualityPanel.classList.add('cp-hidden');
    this._speedPanel.classList.add('cp-hidden');
    if (name === 'main') this._mainPanel.classList.remove('cp-hidden');
    else if (name === 'quality') this._qualityPanel.classList.remove('cp-hidden');
    else if (name === 'speed') this._speedPanel.classList.remove('cp-hidden');
  }
    ...
    this._keyHandler = (e) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'p':
          ...
          break;
        case ',':
          if (e.shiftKey) { this._changeSpeed(-0.25); e.preventDefault(); }
          break;
        case '.':
          if (e.shiftKey) { this._changeSpeed(0.25); e.preventDefault(); }
          break;
  ...
  _changeSpeed(delta) {
    const speeds = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2];
    const curIdx = speeds.indexOf(this._playbackRate);
    const newIdx = Math.max(0, Math.min(speeds.length - 1, curIdx + delta));
    const newRate = speeds[newIdx];
